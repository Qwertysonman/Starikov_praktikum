...
import shutil
import os
from Setting import folder_path as main_folder
...
def rename_file(stdscr, file):
    curses.cbreak() # Режим для обработки символов по одному
...
def main(stdscr):
    ...
    curses.start_color()
    # Задаем цветовую пару (или набор атрибутов цвета)
    ...
    while True:
        try:
            bloak_read = False
            key = stdscr.getch() # Ввод пользователя
